...
# 拼接出绝对路径
BASE_INPUT_DIR = os.path.join(BASE_DIR, "data", "websites")
BASE_OUTPUT_DIR = os.path.join(BASE_DIR, "data", "sorted_intelligence")
...
def call_llm(file_path, target_input_dir):
    """
    新增参数 target_input_dir，用于计算相对路径
    """
    relative_filename = os.path.relpath(file_path, target_input_dir).replace(os.sep, "/")
...
def run_daily_scoring():
    """暴露给 Scheduler 调用的主函数"""
    print(f"\n{'=' * 50}")
    print(f"🤖 [常规情报评分启动] 时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'=' * 50}")
    ...
    target_input_dir = os.path.join(BASE_INPUT_DIR, date_str)
    target_output_dir = os.path.join(BASE_OUTPUT_DIR, date_str)

    if not os.path.exists(target_input_dir):
        print(f"❌ 目录 {target_input_dir} 不存在。退出任务。")
        return
    ...
    for root, dirs, files in os.walk(target_input_dir):
        if "fallback" in dirs:
            dirs.remove("fallback")  # 如果是 fallback 脚本，这里改成只进 fallback
    ...
    print(f"✅ 找到 {len(all_files)} 个文件。开始并发处理...")

    buckets = {f"{i}-{i + 10}.json": [] for i in range(0, 100, 10)}

    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        # 注意这里传参的变化：把 target_input_dir 传给 call_llm
        futures = {executor.submit(call_llm, f, target_input_dir): f for f in all_files}
    ...
    print(f"🎉 任务完成! 结果保存在: {target_output_dir}")
